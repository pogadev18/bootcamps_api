...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // check for user's published bootcamp
  const publishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

  // if you're not an admin, you can't publish more than 1 bootcamp
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // trigger middleware to delete all courses of a specific bootcamp
  bootcamp.remove();
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if (file.size > process.env.MAX_FILE_UPLOAD) {
    return next(new ErrorResponse(`The image size can't be bigger than 10MB`, 400));
  }
